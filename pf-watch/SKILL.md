...
## Why the cron has no `--deliver`

Hermes' `cron --deliver` relays every final response, including the empty ones.
This producer is quiet by design, so it takes no deliver arm and posts for
itself through `pf_shared/pf_chat.py`. If you ever register this cron by hand,
register it without `--deliver`.

## Asked for a check right now

Same command. If it prints `no change`, tell the owner exactly that in one
